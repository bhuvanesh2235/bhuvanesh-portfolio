// scripts/update-year-sort-order.ts
...
async function run() {
  console.log('🔄 Re-ordering Experiences by year ASC...');
  const experiences = await sql`SELECT id, company, role, year FROM experiences ORDER BY year ASC, sort_order ASC, id ASC`;
  for (let i = 0; i < experiences.length; i++) {
    await sql`UPDATE experiences SET sort_order = ${i + 1} WHERE id = ${experiences[i].id}`;
  }

  console.log('🔄 Re-ordering Projects by year ASC...');
  const projects = await sql`SELECT id, slug, title, year FROM projects ORDER BY year ASC, sort_order ASC, id ASC`;
  for (let i = 0; i < projects.length; i++) {
    await sql`UPDATE projects SET sort_order = ${i + 1} WHERE id = ${projects[i].id}`;
  }

  console.log('\n✅ UPDATED EXPERIENCES (ascending by year):');
  ...
  console.log('\n✅ UPDATED PROJECTS (ascending by year):');
